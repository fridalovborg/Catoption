...
---------------------------------------------------------------------------- */
function getIGImages(){var e=new XMLHttpRequest;e.open("GET","getIGdata.php",!0),e.addEventListener("load",function(e){console.log(this.responseText),document.getElementById("img-container").innerHTML=this.responseText}),e.send()}function updateGradient(){if(void 0!==$){var e=colors[colorIndices[0]],o=colors[colorIndices[1]],t=colors[colorIndices[2]],n=colors[colorIndices[3]],r=1-step,s=Math.round(r*e[0]+step*o[0]),l=Math.round(r*e[1]+step*o[1]),c=Math.round(r*e[2]+step*o[2]),a="rgb("+s+","+l+","+c+")",d=Math.round(r*t[0]+step*n[0]),i=Math.round(r*t[1]+step*n[1]),p=Math.round(r*t[2]+step*n[2]),g="rgb("+d+","+i+","+p+")";$("#load-page").css({background:"-webkit-gradient(linear, left top, right top, from("+a+"), to("+g+"))"}).css({background:"-moz-linear-gradient(left, "+a+" 0%, "+g+" 100%)"}),step+=gradientSpeed,step>=1&&(step%=1,colorIndices[0]=colorIndices[1],colorIndices[2]=colorIndices[3],
