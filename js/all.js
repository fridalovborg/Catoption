function bubble(){++bubbleIndex>textInBubbleArray.length-1&&(bubbleIndex=0),document.getElementById("bubble-div").innerHTML=textInBubbleArray[bubbleIndex].innerHTML,TweenMax.fromTo(chatBubbleWhole,.7,{alpha:0},{alpha:1,yoyo:!0,repeat:1,repeatDelay:1})}/* ----------------------------------------------------------------------------
    NOSE
    - makes the nose and whiskers do a small rotation/shake every 5 seconds
---------------------------------------------------------------------------- */
function noseAnimation(e){setInterval(function(){TweenMax.staggerFromTo(e,.2,{rotation:0,transformOrigin:"center center"},{rotation:10,transformOrigin:"center center",yoyo:!0,repeat:1})},5e3)}function eyeBlink(){eyeBlinkInterval=window.setInterval(function(){TweenMax.fromTo(eyesClosed,.3,{alpha:0},{alpha:1,yoyo:!0,repeat:1,repeatDelay:.2}),TweenMax.fromTo([rightEye,leftEye],.3,{alpha:1},{alpha:0,yoyo:!0,repeat:1,repeatDelay:.2})},4e3)}/* ----------------------------------------------------------------------------
    TAIL IS MOVING
    - makes the cats tail sway
---------------------------------------------------------------------------- */
function tailMoving(){setInterval(function(){TweenMax.staggerFromTo(tailAnimation,1,{rotation:0,transformOrigin:"center center"},{rotation:-25,transformOrigin:"center center",yoyo:!0,repeat:1})},4e3)}function highfivePaw(){TweenMax.fromTo(paw,.3,{scale:1,x:0,y:0},{scale:1.3,x:0,y:-15,yoyo:!0,repeat:1}),TweenMax.fromTo($("#highfive-lines"),.5,{alpha:0},{alpha:1,yoyo:!0,repeat:1}),$("#paw-sound").trigger("load"),$("#paw-sound").trigger("play"),
// PAT THE CAT DB COUNTER
addValue()}function pleasedCat(){TweenMax.staggerFromTo([eyebrowRight,mouthRight],2,{rotation:0,transformOrigin:"center center"},{rotation:-30,transformOrigin:"center center",yoyo:!0,repeat:1}),TweenMax.staggerFromTo([eyebrowLeft,mouthLeft],2,{rotation:0,transformOrigin:"center center"},{rotation:40,transformOrigin:"center center",yoyo:!0,repeat:1}),TweenMax.fromTo([rightEye,leftEye],.5,{alpha:1},{alpha:0,yoyo:!0,repeat:1,repeatDelay:2}),TweenMax.fromTo([eyesClosed],.5,{alpha:0},{alpha:1,yoyo:!0,repeat:1,repeatDelay:2}),$("#purr").trigger("load"),$("#purr").trigger("play"),
// PAT THE CAT DB COUNTER
addValue()}function angryCat(){window.clearInterval(eyeBlinkInterval),setTimeout(function(){eyeBlink()},4e3),TweenMax.fromTo(semiClosed,.5,{alpha:0},{alpha:1,yoyo:!0,repeat:1,repeatDelay:2}),TweenMax.staggerFromTo($("#eyebrow-right"),1,{rotation:0,transformOrigin:"center center"},{rotation:-50,transformOrigin:"center center",yoyo:!0,repeat:1,repeatDelay:.5}),TweenMax.staggerFromTo($("#eyebrow-left"),1,{rotation:0,transformOrigin:"center center"},{rotation:60,transformOrigin:"center center",yoyo:!0,repeat:1,repeatDelay:.5})}/* ----------------------------------------------------------------------------
...
$(".img-cont").on("click",function(e){$(this).toggleClass("img-like")}))}),e.send()}function audioBtn(){var e=document.getElementById("sound-icon");player.paused?(player.play(),$(e).toggleClass("volume-up volume-off")):(player.pause(),$(e).toggleClass("volume-off volume-up"))}function initMap(){map=new google.maps.Map(document.getElementById("the-map"),{zoom:15,scrollwheel:!1}),infoWindow=new google.maps.InfoWindow;var e;navigator.geolocation?navigator.geolocation.getCurrentPosition(function(t){
// MY POSITION COORDS
...
google.maps.event.addListener(e,"click",function(){var e="<strong>You are here!</strong><br>";infoWindow.setContent(e),infoWindow.open(map,this)});
// CAT SHELTERS REQUEST
var o={location:n,radius:"50000",keyword:"cat_shelter"};new google.maps.places.PlacesService(map).nearbySearch(o,showPlaces),map.setCenter(n)},function(){handleLocationError(!0,infoWindow,map.getCenter())}):
...
---------------------------------------------------------------------------- */
document.addEventListener("DOMContentLoaded",function(){TweenMax.fromTo(headLayers,.8,{x:-15,y:0},{x:15,y:0,yoyo:!0,repeat:-1}),noseAnimation(nose),noseAnimation(whiskers),eyeBlink(),tailMoving(),bubble()}),/* ----------------------------------------------------------------------------
    PAW
